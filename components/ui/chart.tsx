...
function Indicator({ type, color, nestLabel }: IndicatorProps) {
  const baseClasses = 'shrink-0 rounded-[2px] border-(--color-border) bg-(--color-bg)'
  
  const typeClasses = {
    dot: 'h-2.5 w-2.5',
    line: 'w-1',
    dashed: 'w-0 border-[1.5px] border-dashed bg-transparent my-0.5'
  }

  return (
    <div
      className={cn(baseClasses, typeClasses[type], {
        'my-0.5': nestLabel && type === 'dashed',
      })}
      style={{
        '--color-bg': color,
        '--color-border': color,
